...
const { registerUser } = useAuth(); //bring in loginUser from context
...
const handleRegister = (form: RegisterFormInputs) => {
  ...
  return (
    ...
    <form className="flex flex-col gap-4 p-5 justify-center" onSubmit={handleSubmit(handleRegister)}>
    <label className="input flex items-center gap-2 bg-transparent border border-b-white rounded-none">
      ...
      <button className="border rounded-lg p-2" type='submit' >Register</button>

    </form>
    <p className='pl-5'>Already have an account? Login <Link href='/login' className='underline cursor-pointer'> here</Link></p>
  </div>
  )
}
